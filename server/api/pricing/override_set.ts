...
// Pin the real NET price for a server type (grandfathered/legacy pricing).
...
async function overrideSet(c: Context) {
  ...
  if (!body.serverType || body.monthlyCost == null || body.monthlyCost < 0) {
    return c.json({ error: "serverType and a non-negative monthlyCost are required" }, 400);
  }
  const hourlyCost = body.hourlyCost != null ? body.hourlyCost : body.monthlyCost / 730;
  await db
    .insert(priceOverrides)
    .values({ serverType: body.serverType, monthlyCost: body.monthlyCost, hourlyCost })
    .onConflictDoUpdate({
      target: priceOverrides.serverType,
      set: { monthlyCost: body.monthlyCost, hourlyCost },
    });
  pollOnce().catch(() => {}); // reprice immediately
      ...
      content: {
        "application/json": {
          schema: {
            ...
            required: ["serverType", "monthlyCost"],
          },
        },
      },
    },
